...
class DockerEvaluator:
    ...
    def evaluate(
        self, code, stdin, expected_output, language, time_limit, input_type="stdin", file_io_name="", memory_limit=1024
    ):
        ...
        for language_helper in self.language_helpers:
            if language_helper.language == language:
                output = language_helper.evaluate(
                    code, stdin, time_limit, input_type, file_io_name, memory_limit=memory_limit
                )
