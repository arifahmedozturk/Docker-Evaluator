import os
from docker_evaluator.disk_helper import get_temp_dir, get_cache_dir, get_compile_lock

class LanguageHelper:
    def __init__(self, docker_helper, docker_context_path, language, file_extension, language_time_limit_multiplier, memory_overhead_mb=32, cache_compilation=False):
    ...
    def evaluate(self, code, code_input, time_limit, input_type='stdin', file_io_name='', memory_limit=1024):
        files = [
            {
                "name": f"target.{self.file_extension}",
                "content": code
            },
            {
                "name": "target.in",
        ...
        print(f"env: {environment_variables}, time: {time_limit}s x{self.language_time_limit_multiplier} +{GRACE_S}s grace = {effective_time_limit}s, memory: {memory_limit}KB ({memory_limit_mb}MB) + {self.memory_overhead_mb}MB overhead = {total_memory_mb}MB, container: {compile_safe_memory_mb}MB, cache: {cache_status}")
        ...
        if cache_dir and not os.path.exists(cache_file):
            with get_compile_lock(cache_dir):
                return self.docker_helper.evaluate(self.docker_image_name, temp_dir, environment_variables=environment_variables, memory_limit_mb=compile_safe_memory_mb, cache_dir=cache_dir)
        return self.docker_helper.evaluate(self.docker_image_name, temp_dir, environment_variables=environment_variables, memory_limit_mb=compile_safe_memory_mb, cache_dir=cache_dir)
